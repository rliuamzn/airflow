...
class EmrStartNotebookExecutionOperator(BaseOperator):
    ...
    :param execution_engine: A JSON string to specify the EMR cluster, e.g. {'Id': cluster_id, 'Type': 'EMR'}. (templated)
    :type execution_engine: Optional[str]
    :param service_role: The service role name or ARN needed by the EMR Notebook service to operate on your behalf. (templated)
    :type service_role: Optional[str]
    :param notebook_instance_security_group_id: Security group ID for the EC2 instance that runs the notebook.
        If not supplied, a default security group will be used. (templated)
